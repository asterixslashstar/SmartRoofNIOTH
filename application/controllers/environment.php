...
class environment extends CI_Controller {
    private function _fetch_weather_underground_data()
    {
        $this->load->database();
        ...
        $query_string = 'insert into weather (data) values ("' . $s_string . '");';
    ...
    {
        var_dump("updated");
        print_r($this->_fetch_weather_underground_data());
    }
    ...
    {
        $this->load->database();
        $query_string = "select data from smartroof.weather order by recorded desc limit 1;";
        $query = $this->db->query($query_string);
    ...
    private function _get_conditions()
    {
     ...
     //   $x = type($wind)
        $RH = (int) str_replace("%", "", $relhum);
        $GlobalRad = $solarRadiation;
        $va = min($wind / 3.6, 3);
        
        $es = $this->_calculate_es($Tamb);
        $ehPs = $this->_calculate_ehPa($es, $RH);
        $Tmrt = $this->_calculate_MRT_in_shade($Tamb, $dew_pt, $GlobalRad);
        $sun = $this->_calculate_MRT_in_sun($Tamb, $dew_pt, $GlobalRad);
        return array("shade"=>$Tmrt, "sun"=>$sun);
    }

    private function _calculate_es($Tamb)
    {
        $g = array(-2.8365744E3,-6.028076559E3,1.954263612E1,-2.737830188E-2,1.6261698E-5,7.0229056E-10,-1.8680009E-13,2.7150305);
        $tk = $Tamb + 273.15; // air temp in K
        $es = $g[7] * log($tk);
        for($i = 0; $i < 7; $i++) {
            $es += $g[$i] * pow($tk, $i-2);
        }
    ...
    private function _calculate_MRT_in_sun($Tamb, $dew_pt, $global_rad)
    {
        $T_J = $Tamb;
        $DMRT_LW_J = -11.369 + 0.259 * $dew_pt + 0.00196 * pow($dew_pt,2) + 2;
        $DMRT_SW_J = 0.0464/2 * $global_rad * 1.6;
        $MRT_J = $T_J + $DMRT_LW_J + $DMRT_SW_J;
        return $MRT_J;
    }
    ...
    private function _calculate_ehPa($es, $RH)
    {
        return $es * $RH/100;
    }
    ...
    private function _calculate_approx_UTCI($Ta, $Tmrt, $va, $ehPa )
    {   
        $D_Tmrt = $Tmrt - $Ta;
