...
PWS = "IMILANO61"
PWS_sol = "ILOMBARD268"
http://api.wunderground.com/api/14d8de9b33e35504/conditions/q/pws:IMILANO61.json
def loadWunderground(pws):
    f = urllib2.urlopen('http://api.wunderground.com/api/14d8de9b33e35504/conditions/q/pws:'+pws+'.json')
    json_string = f.read()
...
x = type(wind)
RH = int(relhum [:2])
GlobalRad = float(solarRadiation)
va = min(wind/3.6,3)
...
#in sun
T_J = Tamb
DMRT_LW_J = -11.369 + 0.259 * dew_pt + 0.00196 * math.pow(dew_pt,2) + 2
DMRT_SW_J = 0.0464/2 * GlobalRad * 1.6
MRT_J = T_J + DMRT_LW_J + DMRT_SW_J
#shaded
T_S = Tamb
DMRT_LW_S = -11.369 + 0.259 * dew_pt + 0.00196 * dew_pt ** 2  +3
DMRT_SW_S = 0.0464/2 * GlobalRad  *0.5
MRT_S = T_S + DMRT_LW_S + DMRT_SW_S
#trees + shade
T_V = Tamb - max((Tamb - 20)*0.15,0)
DMRT_LW_V = -11.369 + 0.259 * dew_pt + 0.00196 * dew_pt ** 2 + 2
DMRT_SW_V = 0.0464/2 * GlobalRad * 0.2
MRT_V = T_V + DMRT_LW_V + DMRT_SW_V

ehPa= es(Tamb)*RH/100

def UTCI_approx(Ta,ehPa,Tmrt,va):
    # **********************************************
...
ET_J = UTCI_approx(T_J, ehPa, MRT_J, va)
ET_S = UTCI_approx(T_S, ehPa, MRT_S, va)
ET_V = UTCI_approx(T_V, ehPa, MRT_V, va)

val = str(int(round(ET_J)))+";"+str(int(round(ET_S)))+";"+str(int(round(ET_V)))
print "output is %s " % val
print "location: %s" % location
print "ambient temperature: %s" % Tamb
print "dew point temperature: %s" % dew_pt
print "Relative humidity: %s" % relhum
print "solar radiation: %s" % solarRadiation
print "wind: %s" % wind
print "wind string: %s" % wind_str
print "va: %s" % va
# **********************************************
